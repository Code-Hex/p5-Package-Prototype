...
use Package::Prototype::Checked custom => bless({}, 'Custom');
custom([$x, 'bad']);
SOURCE
...
SKIP: {
    ...
    ($status, $output) = compile_only(<<'SOURCE');
use Package::Prototype::Shape Records => { save => [Dict[id => Int, name => Str]] };
my Records $records;
...
($status, $output) = compile_only(<<'SOURCE');
use Type::Tiny;
use Package::Prototype::Checked custom_array => Type::Tiny->new(
...
for my $expression ('Tuple[Int, (Optional[Str])->create_child_type(constraint => sub {1})]',
    ...
    my $source = 'use Package::Prototype::Checked derived => ' . $expression . ';'
