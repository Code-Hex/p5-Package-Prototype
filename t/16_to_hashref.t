...
my $again = Package::Prototype->to_hashref($object);
...
is(Package::Prototype->to_hashref($object, fields => {}), {}, 'empty selection');
is(Package::Prototype->to_hashref(Package::Prototype->create()), {}, 'empty object');

my $legacy = Package::Prototype->bless({a => [1,2], h => {x => 3},
    u => undef, _hidden => 7, run => sub {die 'must not run'}});
is(Package::Prototype->to_hashref($legacy), {a => [1,2], h => {x => 3}, u => undef},
    'legacy getters use scalar context and exclude private storage');
$legacy->prototype(new_value => 5);
is(Package::Prototype->to_hashref($legacy, fields => {n => 'new_value'}), {n => 5},
...
$parent->set_count(2);
is(Package::Prototype->to_hashref($child), {count => 2}, 'live parent value');
$child->set_count(3);
$parent->set_count(4);
is(Package::Prototype->to_hashref($child), {count => 3}, 'child override');
is(Package::Prototype->to_hashref($grandchild), {count => 3}, 'nearest ancestor value');
is(Package::Prototype->to_hashref($sibling), {count => 4}, 'sibling follows parent');
is(Package::Prototype->to_hashref($parent), {count => 4}, 'parent unaffected');

my $collision = Package::Prototype->derive(parent => $parent, properties => {
    count => {value => 8, reader => 'other_count'},
});
like dies { Package::Prototype->to_hashref($collision) }, qr/Duplicate output key: count/,
    'different declarations cannot silently overwrite';
is(Package::Prototype->to_hashref($collision, fields => {old => 'count', new => 'other_count'}),
...
like dies { Package::Prototype->to_hashref($mixed) }, qr/Duplicate output key: count/,
...
$shadow->set_count(20);
is(Package::Prototype->to_hashref($shadow), {count => 10},
    'own reader retains its value when an inherited writer belongs to another declaration');
my $reserved = Package::Prototype->bless({to_hashref => 6, prototype => 7, '0' => 8});
is(Package::Prototype->to_hashref($reserved), {to_hashref => 6, prototype => 7, '0' => 8},
    'class API does not reserve object member names');
is(Package::Prototype->to_hashref($reserved, fields => {zero => '0'}), {zero => 8},
    'zero is a valid reader name');
my $escaped;
{
    my $temporary = Package::Prototype->create(properties => {items => {value => [9]}});
    $escaped = Package::Prototype->to_hashref($temporary);
}
...
is(Package::Prototype->to_hashref($object, fields => {n => 'get_count'}), {n => 4},
    'writer replacement preserves reader');
$object->prototype(get_count => sub { die 'reader replacement must not run' });
ok !exists(Package::Prototype->to_hashref($object)->{count}), 'replaced reader omitted';
like dies { Package::Prototype->to_hashref($object, fields => {n => 'get_count'}) },
...
is(Package::Prototype->to_hashref($child), {}, 'inherited replacement reflected');

for my $name ('explode', 'set_count', '_private', 'absent', 'prototype') {
    like dies { Package::Prototype->to_hashref($object, fields => {x => $name}) },
        qr/does not name a property reader or value getter/, "reject non-reader $name";
}
for my $invalid (undef, '', [], {}) {
    like dies { Package::Prototype->to_hashref($object, fields => {x => $invalid}) },
        qr/requires a nonempty reader name/, 'invalid reader name';
}
for my $invalid (undef, [], 'count', bless({}, 'Fields')) {
    like dies { Package::Prototype->to_hashref($object, fields => $invalid) },
        qr/fields must be a hash reference/, 'invalid fields';
}
for my $invalid (undef, {}, bless({}, 'Counter')) {
    like dies { Package::Prototype->to_hashref($invalid) },
        qr/Expected a Package::Prototype object/, 'invalid object';
}
like dies { Package::Prototype->to_hashref() }, qr/expects an object and named options/, 'missing object';
like dies { Package::Prototype->to_hashref($object, 'fields') }, qr/expects an object and named options/, 'odd options';
like dies { Package::Prototype->to_hashref($object, unknown => 1) }, qr/Unknown to_hashref option/, 'unknown option';
...
like dies { Package::Prototype->to_hashref($orphan) }, qr/parent has been reblessed/,
    'invalid ancestry raises an exception';

my $cycle = [];
push @$cycle, $cycle;
my $cyclic = Package::Prototype->create(properties => {cycle => {value => $cycle}});
is refaddr(Package::Prototype->to_hashref($cyclic)->{cycle}), refaddr($cycle), 'cycles are not traversed';
pop @$cycle;
...
SKIP: {
    ...
    my $ok = eval q{
        ...
        my $d = Package::Prototype->to_hashref($o);
        is $d->{'名前'}, '日本語', 'Unicode names and values';
        is(Package::Prototype->to_hashref($o, fields => {'表示名' => '読む'}),
...
no_leaks_ok {
    ...
    my $d = Package::Prototype->to_hashref($c);
    eval { Package::Prototype->to_hashref($c, fields => {x => 'absent'}) };
